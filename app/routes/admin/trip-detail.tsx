...
export const Loader = async ({ params }: LoaderFunctionArgs) => {
    const { tripId } = params;
    if (!tripId) throw new Error("Trip ID is required to load trip details.");

    const [trip, trips] = await Promise.all([
        getTripById(tripId), //500
        getAllTrips(4, 0) //500
    ]);
...
const TripDetail = ({ loaderData }: { loaderData: any }) => {
    ...
    return (
        <main className="travel-detail wrapper">
            <Header title={name || "Trip Detail"} description="View and edit AI-Generated travel plans"/>
                        ...
                        <li key={index}>
                            ...
                            <ul>
                                {dayPlan.activities.map((activity, index: number) => (
                                    <li key={index}>
                                        <span className="shrink-0 p-18-semibold">{activity.time}</span>
